...
from utils.depth_utils import calculate_global_depth_scale
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    global_depth_scale, depth_maps = calculate_global_depth_scale(dataset.source_path)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        depth_map = depth_maps[viewpoint_cam.image_name]
        depth_map = torch.from_numpy(depth_map).to("cuda")
        if render_scale > 1:
            pool_kernel_size = render_scale
            # Reshape depth map for PyTorch's pooling functions: [N, C, H, W]
            depth_tensor = depth_map.unsqueeze(0).unsqueeze(0)
            min_pool = nn.MaxPool2d(kernel_size=pool_kernel_size, stride=pool_kernel_size)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.spawn_primitives_from_depth(
                        gt_image,
                        image,
                        depth_map,
                        global_depth_scale,
